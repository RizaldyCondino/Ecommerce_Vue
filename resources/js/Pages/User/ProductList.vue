...
const updateFilterProducts = debounce(() => {
...
watch(selectedBrands, updateFilterProducts, { deep: true })
watch(selectedCategories, updateFilterProducts, { deep: true })
                ...
                <TransitionRoot as="template" :show="mobileFiltersOpen">
                    <Dialog class="relative z-40 lg:hidden" @close="mobileFiltersOpen = false">
                        <TransitionChild as="template" enter="transition-opacity ease-linear duration-300"
                            enter-from="opacity-0" enter-to="" leave="transition-opacity ease-linear duration-300"
                        ...
                        <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                            ...
                            <form class="hidden lg:block">
                                ...
                                <div class="flex items-center justify-between space-x-3">
                                    <div class="basis-1/3">
                                        <label for="filters-prices-from"
                                            class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
                                            From
                                        </label>
                                        <input v-model="filterPrices.prices[0]" type="number" id="filters-price-from"
                                    ...
                                    <div class="basis-1/3">
                                        ...
                                        <input v-model="filterPrices.prices[1]" type="number" id="filters-price-to"
                                            placeholder="Min price"
                                            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900">
                                    </div>
                                    <SecondaryButton class="self-end" :disabled="filterPrices.processing"
                                        @click="priceFilters()">Ok
                                    </SecondaryButton>
                                </div>
                                <!--end-->
                                    ...
                                    <DisclosurePanel class="pt-6">
                                        <div class="space-y-4">
                                            <div v-for="categories in categories" :key="categories.id"
                                                ...
                                                <label :for="`filter-${categories.id}`"
                                                    class="text-sm text-gray-600">{{ categories.name }}</label>
                                            </div>
                                        </div>
                                    </DisclosurePanel>
                                </Disclosure>
